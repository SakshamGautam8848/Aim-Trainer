...
def draw_top_bar(win, elasped_time, target_pressed, misses):
...
def end_screen(win, elasped_time, target_pressed, clicks):
    win.fill(BG_COLOR)
...
def main():
    ...
    while run:
        ...
        for target in targets:
            ...
            if click and target.collide(*mouse_pos): #writing "*" inside a function argument will break down the tuple into its individual component[also known as the splat operator]
                targets.remove(target)
                target_pressed += 1

        if misses >= LIVES:
            end_screen(WIN, elasped_time, target_pressed, clicks) # end game
